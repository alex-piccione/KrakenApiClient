...
module internal mapper =
    ...
    let fetchPairs () = task {
        let! pairsJson = client.GetStringAsync "/0/public/AssetPairs" |> Async.AwaitTask

        return parser.load_result_and_check_errors(pairsJson)
            .Properties()
    ...
    let fetchAssets () = task {
        let! assets = client.GetStringAsync "0/public/Assets" |> Async.AwaitTask
